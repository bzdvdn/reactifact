...
| `current_result()` | последний `ScenarioResult`, произведённый этой задачей — чтобы инструменты (CLI) могли показать состояние прогона, когда падает более поздний ассерт |
