...
## `RollingDigestSummarizer` — одна растущая память вместо чекпоинтов по раундам (§27, §37)

`WindowSummarizer` хранит снимок каждого раунда; но некоторым приложениям
нужна одна память, которая накапливается сама, с небольшим сырым хвостом
поверх. `RollingDigestSummarizer` — эта другая модель: как только диалог
превышает `trigger` сообщений, всё, что старше `window`, сворачивается в
один дайджест-артефакт — каждое сворачивание переписывает дайджест из
*предыдущего* его текста плюс новых выпавших сообщений — а сами свёрнутые
сообщения рецепт удаляет самостоятельно:
...
class Flow(Agent):
    ...
    produces = [
        RollingDigestSummarizer(
            ...
            build=build_digest,
            window=8,    # сколько сообщений остаются сырыми
            trigger=12,  # порог длины диалога, после которого сворачиваем
        ),
    ]
```

- `message_type` принимает как один тип, так и последовательность типов —
  например `[Question, FinalResponse]`, если диалог собран из двух разных
  артефактов, а не одной модели `Msg`. Они объединяются и сортируются по
  `order_key` до того, как считаются window/trigger.
- `summarize(context, previous_digest_text, stale_artifacts) -> str | None`
  получает выпавшие сообщения как сырой список артефактов, а не готовую
  строку — рецепт сам ничего не сплющивает в текст. Это важно, если у вас
  уже есть свой построитель role/content-промпта, или типы сообщений не
  сводятся к одной форме `role`/`text`: рендеринг пишете сами внутри
  `summarize`, и никакого round-trip через строковый формат рецепта не
  происходит. `llm_digest_summarizer(system=...)` — опциональный дефолт для
  тех, кому подходит обычный текстовый промпт: рендерит строками `role:
  text` (переопределяется через `render=`) и оформляет предыдущий дайджест
  и новую историю как две секции одного user-сообщения. `None` от любого из
  путей запускает `fallback` — по умолчанию он дописывает обрезанную строку
  истории к предыдущему дайджесту, а не отбрасывает его.
- `build(text) -> Digest` — форму артефакта дайджеста определяете вы;
  `digest_text` — обратный читатель (по умолчанию `artifact.data.text`), если
  в вашей форме нет поля `text`.
- Дайджест живёт под одним стабильным id (`digest` по умолчанию,
  `digest_id`) — каждое сворачивание обновляет его на месте, а не создаёт
  новый артефакт.
- Не комбинируйте с `WindowPruner`: он удаляет без предварительного
  сворачивания, а значит теряет контент, который этот рецепт должен
  сохранить. Свой сырой хвост он ограничивает сам, ничего больше не нужно.
