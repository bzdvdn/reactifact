...
**Аудируемые агенты для Python.** Стройте **knowledge-ассистентов** по докам,
таблицам и вебу — там, где путь к ответу зависит от вопроса и графом не
ложится. Каждый ответ вычисляется, версионируется и воспроизводим — а не строка,
которой приходится верить.

> **Это:** Python-библиотека (3 core-зависимости) · один процесс · типизированные
> версионируемые артефакты с провенансом · детерминированный replay · работает
> офлайн, без API-ключа
> **Это не:** managed-платформа · распределённая очередь задач · готовый
> маркетплейс агентов/инструментов

Вопрос вроде *«почему в Q2 выросли инфраструктурные расходы?»* требует таблицы,
политики и страницы из веба — а *следующий* вопрос требует уже другого набора.
Это не один граф, который можно нарисовать заранее. Вы объявляете, какие
артефакты существуют и что агенты умеют с ними делать; порядок рантайм выводит из
состояния. Каждый производный артефакт затем ссылается на то, что его создало
(`Answer —supported_by→ Evidence —extracted_from→ Doc`), поэтому *«почему он так
сказал?»* — это запрос, а не догадка, а запуск воспроизводим (`context_hash`) и
проверяем (`reactifact replay --verify`).

<details>
<summary><strong>Если вы знаете Celery…</strong> — модель программирования, а не его распределённый рантайм</summary>
...
контексте, а не на сообщение в очереди и не на ребро графа.
...
**Не наследуется:** брокер, пул воркеров, `acks_late`/redelivery, cross-process
exactly-once — *модель* Celery-образная, деплой — нет.
