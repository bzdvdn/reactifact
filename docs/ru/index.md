...
**Событийные агенты для Python-разработчиков — задачи просыпаются на
типизированных артефактах, как задачи Celery просыпаются на сообщениях. Граф
рисовать не нужно.**
...
Сегодня — один процесс (без брокера и пула воркеров): Celery-образная *модель*,
а не его распределённый рантайм.
